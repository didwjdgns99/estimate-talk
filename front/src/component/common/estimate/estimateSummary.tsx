...
export default function EstimateSummary({
  ...
  return (
    ...
    (
      <section className="flex flex-col w-full rounded-2xl border border-border bg-white p-5">
        <div className="flex flex-col gap-1 border-b border-border pb-2 mb-4">
          <h2 className="mb-2">금액 요약</h2>
          <div className="flex justify-between">
            <span className="text-muted-foreground text-sm">공급가 합계</span>
            <span>{totalOriginalPrice.toLocaleString()} 원</span>
          </div>
          {hasDiscount && (
            <div className="flex justify-between">
              <span className="text-muted-foreground text-sm">할인 합계</span>
