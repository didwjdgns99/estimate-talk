...
async function signupService({ name, email, password }) {
  ...
  if (existUser) {
    const err = new Error(ERROR_MESSAGE.EXISIT_EMAIL);
    err.code = ERROR_MESSAGE.EXISIT_EMAIL;
    err.status = 409;
    throw err;
  }
...
module.exports = { loginService, ERROR_MESSAGE, signupService };
